    ...
    <div className="max-w-4xl mx-auto">
        <div className="text-[14px] prose prose-slate dark:prose-invert max-w-none">
            ...
            components={{
                ...
                ul({children}){
                    return <ul className="list-disc pl6 space-y-2 my-4">
                        {children}
                    </ul>
                },
                 ol({children}){
                    return <ol className=" list-decimal pl6 space-y-2">
                        {children}
                    </ol>
                },
                 li({children}){
                ...
                blockquote({children}){
                    return <blockquote className="border-l-4 border-gray-200 pl4 italic my-4">
                        {children}
                    </blockquote>
                },
                 h1({children}){
                    return <h1 className="text-2xl font-bold mt-6 mb-4">
                        {children}
                    </h1>
                },
                 h2({children}){
                    return <h2 className="rext-xl font-bold mt-6 mb-3">
                        {children}
                    </h2>
                },
                 h3({children}){
                    return <h3 className="text-lg font-bold mt-4 mb-2">
                        {children}
                    </h3>
                },
                 h4({children}){
                    return <h4 className="text-base font-bold mt-4 mb-2">
                        {children}
                    </h4>
                },
                 a({children,href}){
                ...
                thead({children}){
                  ...
                  tr({children}){
                    return <tr className="divide-y divide-gray-200">{
                        children}</tr>
                },
                  th({children}){
                    return <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{
                        children}</th>
                },
                  td({children}){
                    return <td className="px-3 py-2 whitespace-nowrap text-xs">{
                        children}</td>
                },
                  hr(){
